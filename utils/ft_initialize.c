...
{
	if (!((textures[0].img) = mlx_xpm_file_to_image(dis.mlx, param.text_south, &(textures[0].width), &(textures[0].height))))
		return (text_error(1));
	textures[0].addr = mlx_get_data_addr(textures[0].img, &textures[0].bits_per_pixel, &textures[0].line_length, &textures[0].endian);
	if (!(textures[1].img = mlx_xpm_file_to_image(dis.mlx, param.text_north, &textures[1].width, &textures[1].height)))
		return (text_error(2));
	textures[1].addr = mlx_get_data_addr(textures[1].img, &textures[1].bits_per_pixel, &textures[1].line_length, &textures[1].endian);
	if (!(textures[2].img = mlx_xpm_file_to_image(dis.mlx, param.text_east, &textures[2].width, &textures[2].height)))
		return (text_error(3));
	textures[2].addr = mlx_get_data_addr(textures[2].img, &textures[2].bits_per_pixel, &textures[2].line_length, &textures[2].endian);
	if (!(textures[3].img = mlx_xpm_file_to_image(dis.mlx, param.text_west, &textures[3].width, &textures[3].height)))
		return (text_error(4));
	textures[3].addr = mlx_get_data_addr(textures[3].img, &textures[3].bits_per_pixel, &textures[3].line_length, &textures[3].endian);
	if (!(textures[4].img = mlx_xpm_file_to_image(dis.mlx, param.text_sprite, &textures[4].width, &textures[4].height)))
		return (text_error(5));
	textures[4].addr = mlx_get_data_addr(textures[4].img, &textures[4].bits_per_pixel, &textures[4].line_length, &textures[4].endian);
	return (1);
}

static void	all_to_zero()
{
	param.res_x = 0;
	param.res_y = 0;
