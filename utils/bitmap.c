...
static void			calc_bmp_data(t_bmp *bmp)
{
	...
	bmp->bmp_size = 14 + 40 + ((bmp->bytes_width + bmp->pad_size) * g_param.res_y);
...
void			create_image()
{
	...
	while (--bmp.i > 0)
	{
		...
		{
			bmp.r = g_param.game.addr[bmp.i * g_param.game.line_length + bmp.j * g_param.game.bits_per_pixel / 8];
			bmp.g = g_param.game.addr[bmp.i * g_param.game.line_length + bmp.j * g_param.game.bits_per_pixel / 8 + 1];
			bmp.b = g_param.game.addr[bmp.i * g_param.game.line_length + bmp.j * g_param.game.bits_per_pixel / 8 + 2];
